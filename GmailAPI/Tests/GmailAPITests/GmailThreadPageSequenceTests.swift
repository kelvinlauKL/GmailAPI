...
struct GmailThreadPageSequenceTests {
  @Test
  func fetchesOnlyWhenTheCallerRequestsAPage() async throws {
    let tokenProvider = TokenProvider()
    let transport = Transport(responseBodies: [#"{"nextPageToken":"second"}"#])
    let client = GmailClient(tokenProvider: tokenProvider, transport: transport)
    let pages = GmailThreadPageSequence(client: client, request: try GmailThreadListRequest())
  ...
  @Test
  func preservesFiltersAndOpaqueTokensAcrossEmptyPages() async throws {
    let transport = Transport(responseBodies: [
      #"{"threads":[{"id":"first"}],"nextPageToken":"next+/=%2F","resultSizeEstimate":0}"#,
      #"{"threads":[],"nextPageToken":"last","resultSizeEstimate":999}"#,
      #"{"threads":[{"id":"second"},{"id":"first"}],"resultSizeEstimate":999}"#
    ])
    let client = GmailClient(tokenProvider: TokenProvider(), transport: transport)
    let request = try GmailThreadListRequest(
      q: "from:demo+tag@example.com", maxResults: 25, pageToken: "resume+/=%2F",
      labelIds: ["INBOX", "label+/"], includeSpamTrash: true
    )
    let pages = GmailThreadPageSequence(client: client, request: request)
    var threadIDsByPage: [[String]?] = []

    for try await page in pages {
      threadIDsByPage.append(page.threads?.map(\.id))
    }

    #expect(threadIDsByPage == [["first"], [], ["second", "first"]])
    ...
    for (request, pageToken) in zip(requests, ["resume+/=%2F", "next+/=%2F", "last"]) {
      ...
      #expect(components.queryItems == [
        ...
        URLQueryItem(name: "q", value: "from:demo+tag@example.com"),
  ...
  @Test(arguments: ["{}", #"{"threads":[]}"#, #"{"nextPageToken":null}"#, #"{"nextPageToken":""}"#])
  func yieldsTheFinalPageAndStaysFinished(responseBody: String) async throws {
    let transport = Transport(responseBodies: [responseBody])
    let client = GmailClient(tokenProvider: TokenProvider(), transport: transport)
    var iterator = GmailThreadPageSequence(client: client, request: try GmailThreadListRequest()).makeAsyncIterator()
  ...
  func rejectsTokenCyclesBeforeSendingADuplicateRequest(startingToken: String?, continuationTokens: [String]) async throws {
    ...
    let client = GmailClient(tokenProvider: TokenProvider(), transport: transport)
    var iterator = GmailThreadPageSequence(
      client: client, request: try GmailThreadListRequest(pageToken: startingToken)
    ).makeAsyncIterator()

    for _ in continuationTokens { #expect(try await iterator.next() != nil) }
    await #expect(throws: GmailThreadPageSequence.PaginationError.repeatedPageToken) {
  ...
  @Test
  func preservesAnHTTPFailureAfterEarlierPagesAndFinishes() async throws {
    let transport = Transport(
      responseBodies: [#"{"nextPageToken":"second"}"#, "{}"], statusCodes: [200, 503]
    )
    let client = GmailClient(tokenProvider: TokenProvider(), transport: transport)
    var iterator = GmailThreadPageSequence(client: client, request: try GmailThreadListRequest()).makeAsyncIterator()
  ...
  @Test
  func malformedResponsesFinishIteration() async throws {
    let transport = Transport(responseBodies: ["not json"])
    let client = GmailClient(tokenProvider: TokenProvider(), transport: transport)
    var iterator = GmailThreadPageSequence(client: client, request: try GmailThreadListRequest()).makeAsyncIterator()
  ...
  func cancellationFinishesTheIterator(stage: CancellationStage) async throws {
    let tokenProvider = TokenProvider()
    let transport = Transport(responseBodies: [#"{"nextPageToken":"second"}"#], cancelsTask: stage == .duringRequest)
    let client = GmailClient(tokenProvider: tokenProvider, transport: transport)
    let pages = GmailThreadPageSequence(client: client, request: try GmailThreadListRequest())
  ...
  func providesPaginationRecoveryGuidance() throws {
    let error = GmailThreadPageSequence.PaginationError.repeatedPageToken
